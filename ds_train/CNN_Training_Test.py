...
class Process(object):
    def __call__(self, img):
        ...
        if abs(moments['mu02']) < 1e-2:
            ...
            M = np.float32([[1, skew, -0.5 * resizeImgArray.shape[1] * skew], [0, 1, 0]])
            deskewed = cv2.warpAffine(binary, M, (resizeImgArray.shape[1], resizeImgArray.shape[0]),
                                      flags=cv2.WARP_INVERSE_MAP | cv2.INTER_LINEAR)

        # Padding
        desired_size = max(deskewed.shape)
        border = (
        desired_size - deskewed.shape[1], desired_size - deskewed.shape[0], 0, 0)  # (left, right, top, bottom)
        padded = cv2.copyMakeBorder(deskewed, *border, cv2.BORDER_CONSTANT, value=255)

        # Resize to 64x64 using Lanczos interpolation
